...
{
    public class OracleMaterialRemovalController : OracleController, IRecordController
    {
        public async Task<bool> Create(XElement x)
        {
            try
            {
                using (OracleConnection cn = new OracleConnection(base.Connection))
                {
                    try
                    {
                        ...
                        using (var transaction = cn.BeginTransaction())
                        {
                            try
                            {
                                ...
                                var header = OracleTypeHelper.GetHeaderFromMaterialRemoval(i);
                                var hres = await IssueHeaderCommand(header, transaction);
                                var detail = OracleTypeHelper.GetDetailFromMaterialRemoval(i);
                            ...
                            {
                                transaction.Rollback();
                                await SqlErrorHandler.LogError(ex, "MRF-0003 - " + x.ToString());
                            }
                            finally
                            {
                                cn.Dispose();
                    ...
                    {
                        await SqlErrorHandler.LogError(ex, "MRF-0002");
                    }
                }
            }
            catch (Exception ex)
            {
                await SqlErrorHandler.LogError(ex, "MRF-0001");
            }
            return false;
        }
            ...
            using (OracleConnection cn = new OracleConnection(Connection))
            {
                try
                {
                    using (OracleCommand cmd = new OracleCommand(this.OraclePackage, cn))
                    {
                        ...
                        var ru = DeserializeRequest(x);

                        await LogOracleActivityAsync(x.ToString(), ru);
        ...
        protected  async Task<bool> LogOracleActivityAsync(string request, RemovalUpload ru)
        {
            using (var context = new MESFeedClientEFModel.MESFeedClientEntities())
            {
                ...
                if (ru != null)
                {
                    dt = DateTime.Parse(ru.TransactionDateTime);
                    id = ru.TransactionId;
                }
                var w = new tblUploadFeedLog()
                {
                    ...
                    TransactionType = RemovalUpload.TransactionType
        ...
        public RemovalUpload DeserializeRequest(XElement x)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(RemovalUpload));
            using (var a = x.CreateReader())
            {
                return (RemovalUpload)serializer.Deserialize(x.CreateReader());
            }
        }

        private async Task<bool> IssueDetailCommand(OracleMaterialRemovalUploadDetail detail, OracleTransaction tr)
        {
            try
            {
                foreach (var item in detail.GetDetails())
                {
                    string detailquery = string.Format("Insert into XXATR_MES_MR_DETAIL " +
                        " (TRANSACTION_ID, ROUTE_STEP, PRODUCT_NAME, LOT_NUMBER, INVENTORY_LOCATION, QUANTITY, PROCEDURE, ORDER_NUMBER)" +
                        " Values ('{0}','{1}','{2}','{3}','{4}','{5}', '{6}', '{7}')"
                        , detail.TransactionID
                        , item.RouteStepName
                        , item.ProductName
                        , item.LotNumRemoveToInv
                        , item.InvLocToRemoveTo
                        , item.QtyToRemove
        ...
        private async Task<bool> IssueHeaderCommand(OracleMaterialRemovalUploadHeader header, OracleTransaction tr)
        {
            try
            {
                string headersql = string.Format("insert into XXATR_MES_MR_HEADER " +
                    "(TRANSACTION_ID, TRANSACTION_DATE, WORK_ORDER_NAME) " +
